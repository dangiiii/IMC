    ...
    def store(self, m: Measurement):
        with self._redisClient:
            self._redisClient.xadd(self._streamName, {"name": str(m.name), "date": str(m.date), "velocity_f": str(m.velocity_f),
                                                      "velocity_r": str(m.velocity_r), "local_timestamp": str(timeit.default_timer())})
            return True
        return False
...
def main():
    ...
    with client:
        ...
        # set expiry for keys
        # client.expire(stream_name_kpis,1080000)
        ...
        try:
            while True:
                monitorRuns(event_window_size, store, refresh_time_ms, kpi1_threshold, kpi2_seqlength, rtts, stream_name_kpis, stream_name_infos)
